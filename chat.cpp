...
///////////////////////////////////////////////////////////////
	...
	if (!input_noecho) {
    std::ostringstream output_stream;  // Create a string stream to capture output
    for (auto id : embd) {
        // Capture each token and print it to the console
        const std::string token = vocab.id_to_token[id].c_str();
        printf("%s", token.c_str());
        output_stream << token;  // Append the token to the output stream
    }
    fflush(stdout);

    // Now capture the output in a string
    std::string output_text = output_stream.str();
    
    // Use espeak to read the output aloud
    // We use system() to call the espeak command with the output as a parameter
    std::string espeak_command = "espeak \"" + output_text + "\"";
    system(espeak_command.c_str());  // Execute the command to read the output aloud
}
